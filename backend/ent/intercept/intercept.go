...
// The LuminaTaskFunc type is an adapter to allow the use of ordinary function as a Querier.
type LuminaTaskFunc func(context.Context, *ent.LuminaTaskQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f LuminaTaskFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.LuminaTaskQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.LuminaTaskQuery", q)
}

// The TraverseLuminaTask type is an adapter to allow the use of ordinary function as Traverser.
type TraverseLuminaTask func(context.Context, *ent.LuminaTaskQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseLuminaTask) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseLuminaTask) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.LuminaTaskQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.LuminaTaskQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.LuminaTaskQuery:
		return &query[*ent.LuminaTaskQuery, predicate.LuminaTask, luminatask.OrderOption]{typ: ent.TypeLuminaTask, tq: q}, nil
