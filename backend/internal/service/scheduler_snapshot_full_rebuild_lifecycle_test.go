...
func TestSchedulerFullRebuildActiveTombstoneDoesNotBlockFollowingGroupEvent(t *testing.T) {
	...
	expectedTokenCount := 2 * len(canonical)
	require.Len(t, cache.tokens(), expectedTokenCount, "full rebuild and the following group event must each run fresh authority")
	_, reopenHeld := cache.lifecycleMutationLeaseStates()
	require.Len(t, reopenHeld, expectedTokenCount)
...
func TestSchedulerFullRebuildSimpleModePreservesRegistryWithoutLifecycleAuthority(t *testing.T) {
	...
	expectedAccountCalls := len(schedulerSnapshotPlatforms()) + len(registered) + 2
	require.Equal(t, expectedAccountCalls, accounts.callCount())
	require.Equal(t, expectedAccountCalls, accounts.groupCallCount(0))
	require.Empty(t, cache.retiredBuckets())
	require.Empty(t, cache.tokens())
...
func TestSchedulerFullRebuildFreshReopenLockBusyRetriesWithoutBlockingOrdinaryTasks(t *testing.T) {
	...
	expectedCallsPerRound := 2*len(schedulerSnapshotPlatforms()) + 4
	require.Equal(t, expectedCallsPerRound, accounts.callCount())

	svc.pollOutbox()
	require.Equal(t, int64(1), cache.currentWatermark())
	require.Equal(t, 2*expectedCallsPerRound, accounts.callCount())
