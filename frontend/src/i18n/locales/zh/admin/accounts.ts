export default {
    accounts: {
      ...
      lumina: {
        title: 'Lumina 个人订阅',
        description: '保存 BytePlus Lumina Cookie Jar；邮箱和密码仅作为会话失效后的可选兜底，不会绕过验证码、MFA 或安全验证。',
        email: 'BytePlus 邮箱（可选）',
        password: 'BytePlus 密码（可选）',
        passwordFallbackHint: '邮箱和密码必须同时配置。密码按设计明文保存，但 API 永不回显。',
        cookieJar: 'Cookie Jar JSON',
        cookieJarPlaceholder: "[{'{'}\"name\":\"sessionid\",\"value\":\"...\",\"domain\":\".byteplus.com\",\"path\":\"/\"{'}'}]",
        cookieJarHint: '粘贴已登录浏览器导出的 JSON 数组；仅接受 byteplus.com 域 Cookie。',
        cookieJarInvalid: 'Cookie Jar 必须是有效 JSON 数组，且每项包含 name、value 和 domain。',
        sharkWebId: 'x-shark-web-id（可选）',
        sharkWebIdHint: '仅当抓取的 Lumina 请求使用固定 x-shark-web-id 时填写。',
        emailPasswordTogether: 'Lumina 邮箱和密码必须同时配置。',
        credentialsRequired: '请配置 Cookie Jar，或同时配置 Lumina 邮箱和密码。',
        modelMappingHint: '将对外 ModelArk 模型 ID 映射到 Lumina req_key；创建账号后可同步实时目录。',
        keepCookieHint: '留空则保留已保存的 Cookie Jar。',
        keepPasswordHint: '留空则保留已保存的密码。',
      },
      ...
      mediaPromptLabel: '生成提示词',
      mediaPromptPlaceholder: '例如：一只戴宇航员头盔的橘猫漂浮在柔和的星云中。',
      mediaPromptDefault: 'A cute orange cat astronaut floating in a pastel nebula.',
      mediaTestHint: '选择图片或视频模型后，这里会发起一次真实生成，并在下方展示结果。视频生成较慢，最长等待约 3 分钟。',
      mediaTestMode: '模式：媒体生成测试',
      sendingMediaRequest: '发送媒体生成测试请求...',
      videoPreview: '生成视频：',
      videoReceived: '已收到测试视频',
      // Stats Modal
      viewStats: '查看统计',
